...
class ApiGetter:
    ...
    def fill_photo_info(self, photo_form, api_provider):
        if api_provider == 'unsplash':
            ...
            self.photo_info['author_name'] = os.path.dirname(os.path.abspath(__file__))
    ...
    def pick_photo(self, api_provider, tag, orientation, color):
        ...
        try:
            with open(os.path.dirname(os.path.abspath(__file__)) + f'/jsons/{api_provider}_{tag}_{orientation}_{color}.json', 'rt') as file:
    ...
    def store_json(self, json_form, api_provider, tag, orientation, color):
        with open(os.path.dirname(os.path.abspath(__file__)) + f'/jsons/{api_provider}_{tag}_{orientation}_{color}.json', 'at') as file:
