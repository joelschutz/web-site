...
class ApiGetter:
    ...
    def unsplash(self):
        ...
        color = f'&color={colors[self.color]}' if self.color is not 'all' else ''
        orientation = f'&orientation={orientations[self.orientation]}' if self.orientation is not 'all' else ''
    ...
    def get_response(self):
        if self.limit > 5:
            return eval(self.provider.lower())()
        else:
